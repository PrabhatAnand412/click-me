...
export function applyTheme(name) {
  const [background, sidebarShadow, arenaShadow, arenaBackground] =
    styles[name] || styles.default;
  document.body.style.background = background;
  el.sidebar.style.boxShadow = sidebarShadow;
  el.gameArena.style.boxShadow = arenaShadow;
  el.gameArena.style.background = arenaBackground;
}
export function updateThemeUI() {
  [...Object.keys(themeCosts), "default"].forEach((name) => {
    el[name + "Theme"].textContent =
      state.selectedTheme === name
        ? "Selected"
        : state.ownedThemes.includes(name)
          ? "Owned"
          : "Buy";
  });
  if (state.ownedThemes.length >= 3) unlockAchievement("Theme Collector");
}
function selectTheme(name) {
  const cost = themeCosts[name];
  if (cost && !state.ownedThemes.includes(name)) {
    if (state.coins < cost) return;
    state.coins -= cost;
    state.ownedThemes.push(name);
  }
  state.selectedTheme = name;
  save.set("coins", state.coins);
  save.set("selectedTheme", name);
  save.set("ownedThemes", state.ownedThemes);
  applyTheme(name);
  renderHUD();
  updateThemeUI();
  renderStatistics();
...
export function initialiseThemes() {
  Object.keys(themeCosts).forEach((name) =>
    el[name + "Theme"].addEventListener("click", () => selectTheme(name)),
  );
  el.defaultTheme.addEventListener("click", () => selectTheme("default"));
}
