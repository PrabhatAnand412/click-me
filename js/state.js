...
const number = (key, fallback) =>
  Number.parseInt(save.get(key, fallback), 10) || fallback;
...
export const state = {
  ...
  highestLevel: number("highestLevel", 1),
  coins: number("coins", 0),
  highScore: number("highScore", 0),
  gamesPlayed: number("gamesPlayed", 0),
  lifetimeClicks: number("lifetimeClicks", 0),
  lifetimeCoins: number("lifetimeCoins", 0),
  ownedSkins: array("ownedSkins", ["default"]),
  selectedSkin: save.get("selectedSkin", "default"),
  ownedThemes: array("ownedThemes", ["default"]),
  selectedTheme: save.get("selectedTheme", "default"),
  achievements: array("achievements", []),
  currentDifficulty: ["easy", "normal", "hard", "nightmare"].includes(
    difficulty,
  )
    ? difficulty
    : "normal",
...
export const config = {
  xpNeeded: 100,
  rewardTable: [50, 100, 150, 200, 250, 300, 500],
  achievements: [
    "First Catch",
    "Button Hunter",
